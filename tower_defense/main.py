...
# Важные переменные
game_over = False
game_outcome = 0  # -1=проигрыш, 1=победа
level_started = False
...
def draw_text(text, font, text_col, x, y):
    ...
    Args:
        ...
        font (pg.font.Font): Шрифт для текста.
        text_col (str): Цвет текста.
        x (int): Позиция X для отображения текста.
        y (int): Позиция Y для отображения текста.
    """
...
def create_turret(mouse_pos):
    """Создание новой турели на карте.

    Args:
        mouse_pos (tuple): Координаты курсора мыши.
    """
...
def clear_selection():
    """Очистка выделения всех турелей."""
    for turret in turret_group:
...
while run:
    ...
    for event in pg.event.get():
        ...
        if event.type == pg.MOUSEBUTTONDOWN and event.button == 1:
            ...
            #print(mouse_pos)
            # если мышка в области игры
            if mouse_pos[0] < c.SCREEN_WIDTH and mouse_pos[1] < c.SCREEN_HEIGHT:
                # очистка выбранных турелей
                selected_turret = None
                clear_selection()
                if placing_turrets == True:
                    # проверка баланса для покупки турели
